...
export default function StockistOrdersPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to load stockist orders');
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = [
    ...
    { key: 'user', label: 'Buyer', render: (r) => r.user?.name || '—' },
  ...
  return (
    <div>
      <PageHeader
        title="Stockist Orders"
        subtitle="Review unpaid orders, confirm payment, then mark Packed → Dispatched → Delivered"
      />
