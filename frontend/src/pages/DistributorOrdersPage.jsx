...
export default function DistributorOrdersPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to load distributor orders');
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = [
    ...
    { key: 'user', label: 'Distributor', render: (r) => r.user?.name || '—' },
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Review unpaid orders, confirm payment, then mark Packed → Dispatched → Delivered (same flow as admin Stockist Orders)"
      />
